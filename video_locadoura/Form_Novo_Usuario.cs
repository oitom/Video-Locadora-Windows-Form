...
{
    public partial class Form_Novo_Usuario : Form
    {
            ...
            { 
                ...
                if (Resultado == System.Windows.Forms.DialogResult.Yes)
                {
                    try
                    {
                        ...
                        SqlCommand COMANDOSELECT_LOGIN = new SqlCommand("SELECT USU_Login FROM USUARIO", CONEXAO);
                        LEITOR = null;
                        LEITOR = COMANDOSELECT_LOGIN.ExecuteReader();
                        //Verificação se já existe um login igual ao informado
                        while (LEITOR.Read())
                        {
                        ...
                        {
                            //Cadastra o Usuário
                            SqlCommand COMANDOINSERIR_USU = new SqlCommand("INSERT INTO  USUARIO VALUES (@USU_Nome, @USU_RG, @USU_CPF, @USU_Dt_Adim, @USU_Sexo, @USU_Foto, @USU_Login, @USU_Senha)", CONEXAO);
                            ...
                            SqlParameter PUSUSEXO = new SqlParameter();
                            if (radioButtonFem.Checked == true)
                                PUSUSEXO = new SqlParameter("@USU_Sexo", "F");
                            else
                                PUSUSEXO = new SqlParameter("@USU_Sexo", "M");

                            SqlParameter PUSUFOTO = new SqlParameter();
                            //Define a foto padrão do usuário
                            if (procurar == null)
                            {
                                if (radioButtonMasc.Checked == true)
                                    PUSUFOTO = new SqlParameter("@USU_Foto", @"C:\PROGRAMA LOCADORA\BANCO DE DADOS\IMAGENS USUARIO\masculino.JPG");
                                else
                                    PUSUFOTO = new SqlParameter("@USU_Foto", @"C:\PROGRAMA LOCADORA\BANCO DE DADOS\IMAGENS USUARIO\feminino.JPG");

                            }
                            else
                            {
                                PUSUFOTO = new SqlParameter("@USU_Foto", procurar.FileName.ToString());
                                pictureBoxFoto.Tag = procurar.FileName.ToString();
                            }
                            ...
                            COMANDOINSERIR_USU.Parameters.Add(PUSUSEXO); COMANDOINSERIR_USU.Parameters.Add(PUSUFOTO);
                        ...
                        {
                            //Mostra um erro que o login digitado já existe
                            erro_login = true;
